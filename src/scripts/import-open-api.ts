 ...
 *
 * @param path abosulte path of the yaml/json openapi file
 */
const importSpecs = (path: string): Promise<OpenAPIObject> => {
  const data = readFileSync(path, "utf-8");
  const { ext } = parse(path);
  const schema = [".yaml", ".yml"].includes(ext.toLowerCase()) ? YAML.parse(data) : JSON.parse(data);
